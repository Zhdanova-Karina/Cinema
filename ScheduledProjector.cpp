...
void ScheduledProjector::showSchedule() const {
    lock_guard<mutex> lock(scheduleMutex);
    cout << "  [Планировщик] Расписание задач:" << endl;
    if (schedule.empty()) {
        ...
        for (size_t i = 0; i < schedule.size(); ++i) {
            ...
            cout << (task.completed ? " [Выполнено]" : " [Ожидает]") << endl;
        }
    }
}
...
void ScheduledProjector::turnOn() {
    cout << "  [Планировщик] Команда включения" << endl;
    wrappedProjector->turnOn();
}

void ScheduledProjector::turnOff() {
    cout << "  [Планировщик] Команда выключения" << endl;
    wrappedProjector->turnOff();
}

void ScheduledProjector::play(const string& fileName) {
    cout << "  [Планировщик] Команда воспроизведения" << endl;
...
string ScheduledProjector::getStatus() const {
    ...
    lock_guard<mutex> lock(scheduleMutex);  // Синхронизация для доступа к schedule
    status += "\n  [Планировщик] Задач в расписании: " + to_string(schedule.size());
